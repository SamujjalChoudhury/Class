...
void hex2string(uint64_t input[], int len)
{
	int i, j;
	char result[len * 8];
...
int main(int argc, const char * argv[]) 
{
	...
	int loop, i, j, len=strlen(str);;
	if(len%8==0){
		loop=len/8;
	} else {
		loop=(len/8)+1;
	}
	uint64_t input[loop], result[loop], temp[loop];
	
	for(j=0;j<loop;j++){
		for(i=0;i<8;i++){
			input[j] = input[j] * 0x100 + str[j*8+i];
		}
	}
	...
	for(i=0;i<loop;i++){
	    ...
	    result[i] = des(input[i], key[0], 'e');
	    result[i] = des(result[i], key[1], 'd');
	    result[i] = des(result[i], key[2], 'e');
	    temp[i] = result[i];   
	}
    printf("3DES: ");
	hex2string(temp, loop);
    
    
    printf ("\n\nDecrypt 3-DES\n");   
    for(i=0;i<loop;i++){
	    result[i] = des(result[i], key[2], 'd');
	    result[i] = des(result[i], key[1], 'e');
	    result[i] = des(result[i], key[0], 'd');
	}
    printf ("3Des: ", result[0], result[1]);
    hex2string(result, loop);
